...
// No SIMD intrinsics, no parallel iterators, no FMA.  Compiled with
// `-C target-feature=-avx,-avx2,-fma` (see build.rs in Stylus crate).
...
///   fc1_w [512 × 784], fc1_b [512],
///   fc2_w [128 × 512], fc2_b [128],
///   fc3_w [10  × 128], fc3_b [10]
pub fn init(path: &str) -> Result<(), String> {
    let data = std::fs::read(path)
        .map_err(|e| format!("krain-aivm: failed to read {path}: {e}"))?;

    let expected = (FC1_OUT * FC1_IN
        + FC1_OUT
        + FC2_OUT * FC1_OUT
        + FC2_OUT
        + FC3_OUT * FC2_OUT
...
// ── Public API — called by crates/stylus/src/aivm.rs ──────────────────────────────────────────────────────────────────────
...
/// Step 2: ReLU activation — [512 × f32] → [512 × f32].
pub fn relu_512(_model_id: u64, pre: &[u8]) -> Result<Vec<u8>, &'static str> {
    if pre.len() != FC1_OUT * 4 {
        return Err("krain-aivm: relu_512: wrong input length");
    }
    let input = decode_f32_le(pre);
    let mut output = Vec::new();
...
/// Step 3: Dropout no-op (eval mode) — [512 × f32] → [512 × f32], identity.
pub fn dropout_noop_512(_model_id: u64, pre: &[u8]) -> Result<Vec<u8>, &'static str> {
    if pre.len() != FC1_OUT * 4 {
        return Err("krain-aivm: dropout_noop_512: wrong input length");
    }
    Ok(pre.to_vec())
}

/// Step 4: FC2 linear — [512 × f32] → [128 × f32].
pub fn fc2_linear(_model_id: u64, pre: &[u8]) -> Result<Vec<u8>, &'static str> {
    let w = weights().ok_or("krain-aivm: not initialised")?;
    if pre.len() != FC1_OUT * 4 {
        return Err("krain-aivm: fc2_linear: wrong input length");
    }
    let input = decode_f32_le(pre);
    let mut output = Vec::new();
...
/// Step 5: ReLU activation — [128 × f32] → [128 × f32].
pub fn relu_128(_model_id: u64, pre: &[u8]) -> Result<Vec<u8>, &'static str> {
    if pre.len() != FC2_OUT * 4 {
        return Err("krain-aivm: relu_128: wrong input length");
    }
    let input = decode_f32_le(pre);
    let mut output = Vec::new();
    relu(&input, &mut output);
...
/// Step 6: Dropout no-op (eval mode) — [128 × f32] → [128 × f32], identity.
pub fn dropout_noop_128(_model_id: u64, pre: &[u8]) -> Result<Vec<u8>, &'static str> {
    if pre.len() != FC2_OUT * 4 {
        return Err("krain-aivm: dropout_noop_128: wrong input length");
    }
    Ok(pre.to_vec())
}

/// Step 7: FC3 linear — [128 × f32] → [10 × f32] logits.
pub fn fc3_linear(_model_id: u64, pre: &[u8]) -> Result<Vec<u8>, &'static str> {
    let w = weights().ok_or("krain-aivm: not initialised")?;
    if pre.len() != FC2_OUT * 4 {
        return Err("krain-aivm: fc3_linear: wrong input length");
    }
    let input = decode_f32_le(pre);
    let mut output = Vec::new();
...
/// Step 8: Argmax + encode — [10 × f32] → [digit_u8 | 10 × f32 LE] = 41 bytes.
pub fn argmax_result(_model_id: u64, pre: &[u8]) -> Result<Vec<u8>, &'static str> {
    if pre.len() != FC3_OUT * 4 {
        return Err("krain-aivm: argmax_result: wrong input length");
    }
